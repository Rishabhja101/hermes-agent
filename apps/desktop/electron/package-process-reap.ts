 ...
 *
 * Scoping and safety. The match is deliberately the CURRENT install root, not
 * the package family: once this ships, every shutdown reaps our own processes,
 * so a stale sibling version can never be left behind to pin the silo. That
 * keeps the predicate to processes this install owns. Cleaning up an ALREADY
 * leaked pinner from a previously-installed version needs an out-of-package
 * recovery path (the app cannot launch to clean up after itself) and is tracked
 * separately.
 *
...
export interface ReapPackageRootedProcessesDeps {
  /**
   * Absolute install root to scope on (Electron's `process.resourcesPath`
   * resolves inside it). Null/empty disables the reap entirely.
   */
...
/**
 * True when `imagePath` lives under `root`.
 *
 ...
 * sibling directory whose name merely begins with the root
 ...
 */
export function isUnderInstallRoot(imagePath: string | null | undefined, root: string | null | undefined): boolean {
  if (!imagePath || !root) {
    return false
  }
...
export function reapPackageRootedProcesses(deps: ReapPackageRootedProcessesDeps): ReapOutcome {
  ...
  for (const candidate of running) {
    ...
    if (!isUnderInstallRoot(candidate.path, deps.installRoot)) {
      continue
    }
